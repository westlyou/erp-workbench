...
alias  %(urname)sc="cd %(ur)s/%(proname)s_addons; do_checkit"
alias gogo="for x in *; do cd \$x; echo ----------; echo \$x; echo ----------; git pull; cd ..; done"
"""
PROS = """# projects
...
def main():
    ...
    for line in data.split('\n'):
        if line.find('alias doc="') > -1:
            docfound = True
        if line.find('alias pro="') > -1:
            profound = True
        if line.find('alias  do_checkit') > -1:
            checkitfound = True
    ...
    if not checkitfound:
        newdata += CHECKIT
    if not docfound:
        newdata += DOC % {'user_home' : os.path.expanduser("~/")}
    #print newdata
